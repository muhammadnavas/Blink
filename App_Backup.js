...
export default function App() {
  ...
  const initializeApp = async () => {
    ...
    // Add listener for when notifications are actually received/displayed
    const receivedSubscription = Notifications.addNotificationReceivedListener(notification => {
      const receivedTime = Date.now();
      console.log(`\n🔔 ===== NOTIFICATION APPEARED =====`);
      console.log(`🔔 APPEARED AT: ${new Date(receivedTime).toLocaleString()}`);
      console.log(`🔔 TITLE: ${notification.request.content.title}`);
      console.log(`🔔 BODY: ${notification.request.content.body}`);
      
      // Check timing if this was one of our tracked notifications
      const data = notification.request.content.data;
      if (data && data.logId) {
        console.log(`🔔 LOG ID: ${data.logId}`);
        if (data.scheduledAt) {
          const actualDelay = (receivedTime - data.scheduledAt) / 1000;
          const expectedDelay = data.expectedDelay || 0;
          console.log(`🔔 ⏰ TIMING: Expected ${expectedDelay}s, actual ${actualDelay.toFixed(1)}s`);
          console.log(`🔔 ⏰ DIFFERENCE: ${(actualDelay - expectedDelay).toFixed(1)}s`);
          
          if (Math.abs(actualDelay - expectedDelay) < 5) {
            console.log(`🔔 ✅ TIMING OK: Within 5 seconds of expected`);
          } else {
            console.log(`🔔 ❌ TIMING ISSUE: More than 5 seconds off`);
          }
        }
      }
      console.log(`🔔 =====================================\n`);
  ...
  const testOneHourTiming = async () => {
    console.log('� === SIMPLE TIMING TEST ===');
    ...
    try {
      // Test 1: 30 seconds
      console.log('\n🧪 Testing 30 seconds...');
      const id30 = await scheduleLocalNotification('30 Second Test', 'This should appear in 30 seconds', 30);
      
      // Test 2: 1 minute  
      console.log('\n🧪 Testing 1 minute...');
      const id60 = await scheduleLocalNotification('1 Minute Test', 'This should appear in 1 minute', 60);
      
      // Test 3: 5 minutes
      console.log('\n🧪 Testing 5 minutes...');
      const id300 = await scheduleLocalNotification('5 Minute Test', 'This should appear in 5 minutes', 300);
      
      Alert.alert(
        'Simple Tests Scheduled',
        `30s: ${id30}\n1m: ${id60}\n5m: ${id300}\n\nIf these appear immediately, the issue is with Expo Go itself.`
      );
      ...
      switch (item.type) {
        ...
        case 'interval':
          return item.intervalMinutes ? 
            ...
            : 'Interval';
        default:
          return formatTimeDisplay(item.time || 5);
      }
    };
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.customTimeContainer}>
              <View style={styles.timeInputGroup}>
                ...
                <View style={styles.timeInputContainer}>
                  ...
                  <Text style={styles.timeValue}>{customHours}</Text>
                  <TouchableOpacity 
                    style={styles.timeButton}
              ...
              <View style={styles.timeInputGroup}>
                ...
                <View style={styles.timeInputContainer}>
                  ...
                  <Text style={styles.timeValue}>{customSeconds}</Text>
                  <TouchableOpacity 
                    style={styles.timeButton}
            ...
            <View style={styles.customTimePreview}>
              <Text style={styles.customTimePreviewText}>
                Total: {customHours}h {customMinutes}m {customSeconds}s
              </Text>
              <Text style={styles.customTimePreviewSubtext}>
